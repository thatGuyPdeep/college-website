        ...
        <StaggerReveal className="flex flex-wrap justify-center gap-2">
          {SOCIALS.map(({ label, href, icon: Icon }) => (
            <a
              ...
              className="micro-lift micro-press inline-flex items-center gap-2 px-4 py-2 rounded-full border border-blue-100 bg-[#F0F4FF] text-sm font-medium text-[#0D2660] hover:border-[#0D2660]/30 hover:bg-white transition-colors group"
            >
              <Icon className="h-4 w-4 text-[#C8201A] micro-icon-pop" aria-hidden="true" />
              {label}
            </a>
          ))}
        </StaggerReveal>
