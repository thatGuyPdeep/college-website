      ...
      <div className={compact ? "" : "container-site py-4 sm:py-5"}>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3 sm:mb-4">
          ...
          <Link
            ...
            className="text-xs font-bold uppercase tracking-wide text-white hover:text-[#F5C200] shrink-0"
          >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
          {ADMISSION_PROGRAMME_STATS.map((p) => (
            <div
              key={p.slug}
              className="border border-white/10 bg-white/5 px-3 py-2.5 text-center"
            >
              <p className="text-xl sm:text-2xl font-bold text-white">{p.uniqueApplicants}</p>
